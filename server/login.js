...
module.exports = function (app, callback) {
  if (intentDictionary.enabled === 1) {
    app.intent(intentDictionary.name, { slots: intentDictionary.slots, utterances: intentDictionary.utterances },
      function (req, res) {
        const eventInstance = new eventAnalytics.event(intentDictionary.name);
        return app
          .prep(req, res)
          .then(function (req) {
            ...
            const voiceMessage = !app.isReprompt(req) ? req.slot('COMMANDREQ') : 'ok';
            ...
            eventInstance.sendAV([zone, service.name, command.avCommand]);
          })
          .fail(function (err) {
